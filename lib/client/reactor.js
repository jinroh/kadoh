  ...
  KadOH.Reactor = Class({
    ...
    _handler: function(data) {
      ...
      // we send it right back as an error
        ...
        message = Protocol.buildErrorResponse(RPCError);
    ...
    _handleRPC: function(src, request) {
      ...
      try {
        var promise = method.apply(this._node, request.getParams());
        ...
        response = Protocole.buildErrorResponse(RPCError, request.getRPCID());
      }
    },
