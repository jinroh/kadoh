...
(function(exports) {
  ...
  KadOH.Reactor = EventEmitter.extend({
    ...
    initialize: function(node, options) {
      ...
      this._udp = new SimUDP(globals._transport_server, (options) ? options.transport : undefined);
    ...
    connectTransport : function() {
      ...
      var self = this;
      this._udp.on('connect', function(iam) {
        self.transport_state = 'connected';
        self.emit('transport connect', iam);

      // main listen loop
        self._udp.listen(bind(self._handler, self));
      });
