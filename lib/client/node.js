...
(function(exports) {
  ...
  KadOH.Node = Peer.extend({
    ...
    /**
     * Set a given value on the DHT associated to the given key and
     ...
     * @return {self}
     */
    set: function(key, value, exp, callback, context) {
      ...
      //default values
      key = (key && key !== null) ? key : Crypto.digest.SHA1(String(value));
      exp = exp || -1;
      context = context || this;

      this.iterativeStore(key, value, exp)
          .then(function(key, peers) {
            if(callback) callback.call(context, key, peers.length());
          }, function() {
...
// # INTERNAL EVENTS HANDLING
    ...
    VMEvents : {
      ...
      republish: function(key, value, exp) {
        this.iterativeStore(value, key, exp);
      }
    },
