...
(function(exports) {
  ...
  KadOH.Node = Peer.extend({
    ...
    reactorEvents : {
      ...
      /**
       * On `outdated` (means we recived a response from a peer
    ...
    /**
     * Handle an icoming STORE request :
     * sotore the value in ValueManagement and respond.
     *
    ...
    VMEvents : {
       ...
       * on the network ) : do an itrative store on it.
       *
    ...
    /**
     * TODO : Description
     * TODO : expose a public API and add callback.
     *
    ...
    /**
     * TODO : Description
     * TODO : expose public API with callback
     *
      ...
      return this.iterativeFindNode(key).pipe(
        function(peers) {
          ...
          return Deferred.whenAtLeast(this._reactor.sendRPCs(targets, 'STORE', key, value, exp), 1)
                         .pipe(function(stored, not_stored) {
 
                           var peers_stored = new PeerArray(stored.map(function(rpc){
                             return rpc.getQueried();
                           }));
 
                           var peers_not_stored = new PeerArray(not_stored.map(function(rpc){
                             return rpc.getQueried();
                           }));
                           ...
                           return new PeerArray(not_stored.map(function(rpc){
                             return rpc.getQueried();
                           }));
                         });
        },
