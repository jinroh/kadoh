...
(function(exports) {
  ...
  var Crypto = KadOH.util.Crypto = {
    ...
    compareHex: function(a, b) {
      ...
      for (var c = 0; c < a.length; c += 2) {
        ...
        byte_b = parseInt(a.substr(c, 2), 16);
        if (byte_a > byte_b)
          return 1;
    ...
    distance: function(hex1, hex2) {
      hex1 = Array.isArray(hex1) ? Crypto.bytesToHex(hex1) : hex1;
      hex2 = Array.isArray(hex2) ? Crypto.bytesToHex(hex2) : hex2;
      ...
      if (length != hex2.length)
        throw new TypeError('error while calculating distance of different length string');
      ...
      for (var c = 0; c < length; c+=2) {
        ...
        if (diff > 0)
          return 4*(length - c) + Math.floor(Math.log(diff) / log2) - 7;
      }
      return 0;
    }
  };
  ...
  Crypto.digest = {
    ...
    randomSHA1: function(message) {
      var bytes = [];
      for (var i = 0; i < 40; i++) {
        bytes.push(Math.floor(Math.random() * 256));
      }
      return Crypto.digest.SHA1(Crypto.bytesToHex(bytes));
    },
