...
(function(exports) {
  ...
  var CustomPeerArray = KadOH.PeerArray.extend({
  ...
  KadOH.IterativeFind = Deferred.extend({
    ...
    initialize : function(nodeInstance, target, targetType) {
      ...
      this.Reached    = new XORSortedPeerArray().setRelativeNodeId(this._target.getID()); // Peers queried that successfully responded
      this.NotReached = new XORSortedPeerArray().setRelativeNodeId(this._target.getID()); // Peers queried but failed to respond
      this.Trap       = new XORSortedPeerArray().setRelativeNodeId(this._target.getID()); // Peers queried but that appears unrelevant to query

      this.emit('init');
    },

    startWith : function(peers) {
      this.HeardOf.add(peers).sortByDistanceTo(this._target.getID());
    ...
    handleNewHeardOf : function(newHeardOf) {
      ...
      this.HeardOf.add(newHeardOf)
                  .sortByDistanceTo(this._target.getID());

      if (this.HeardOf.sortMadeNewClosestTo(this._target.getID())) {

        var toQuery = this.HeardOf.sortByDistanceTo(  this._target.getID())
                                .pickOutFirst(        globals.ALPHA);
    ...
    checkIfWeGotIt : function() {
      ...
      try {
        closest_reached_peer = this.Reached
                                   .sortByDistanceTo(this._target.getID())
                                   .getPeer(0);
      } catch(e){}
    ...
    checkIfItsFinished : function() {
      ...
      this.HeardOf.sortByDistanceTo(this._target.getID());
    ...
    sendFindRPC : function(peers) {
      ...
      this.Queried.add(peers).sortByDistanceTo(this._target.getID());
