...
(function(exports) {
  ...
  KadOH.IterativeFind = Deferred.extend({
    ...
    initialize : function(node_instance, target_id, target_type) {
      this.supr();
    ...
    _lookupSucceeded: function() {
      return (
        this.Reached.getPeer(0).getId() === this._targetId ||
        (
          !this.HeardOf.sortMadeNewClosestTo() &&
          this.Reached.length() >= globals._k
        )
      );
    },
