...
(function(exports) {
  ...
  KadOH.IterativeFind = Deferred.extend({
    ...
    _startIteration: function() {
      this.HeardOf.pickOutFirst(globals.ALPHA)
    ...
    _staleProtection: function() {
      var peers = this.HeardOf.filterOut(this.Queried);
      if (peers.empty()) {
        this.reject(this.Reached);
      } else {
        peers.pickOutFirst(globals.K)
             .sendThemFindRPC(this);
        this.progress();
      }
    },
    ...
    _lookupFinished: function() {
      var finished = (this.Querying.empty() && this.Reached.length() >= globals.K);
    ...
    _lookupStaled: function() {
      var staled = (this.Querying.empty() && this.Reached.length() < globals.K);
      if (this._newClosest !== null) {
    ...
    sendFindRPC: function(peers) {
      var new_requests = this._node.reactor().sendRPCs(peers, this._targetType, this._target);
