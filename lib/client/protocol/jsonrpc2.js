...
(function(exports) {
  ...
  var jsonrpc2 = KadOH.protocol.jsonrpc2 = {
    ...
    parseRequest : function(request) {
      try {
        ...
        request.id = String(request.id); // id should be strings
                  
        // convert params
        if (request.params === undefined || request.params === null)
          request.params = [];
        else if (typeof request.params === 'string' || typeof request.params === 'object')
  ...
  var rpcError = function(code, message, data) {
    var error = {
      code : code,
      message : message ? message : (jsonrpc2.JSONRPC_ERROR_STRINGS[code] ? jsonrpc2.JSONRPC_ERROR_STRINGS[code] : '')
    };
    if (data !== undefined)
      error.data = data;
    return error;
