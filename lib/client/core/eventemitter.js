...
(function(exports) {
  ...
  KadOH.core.EventEmitter = Class(
    ...
    on: function(type, listener, scope, once) {
      ...
      } else if (this._fired(event) && event.memory) {
        event.firingStart = length - 1;
        this._fire(event, event.memory);
      }
    ...
    removeListener: function(type, listener) {
      if (this._events.hasOwnProperty(type)) {
        var event     = this._events[type],
            callbacks = this._events[type].callbacks;
    ...
    */
    emit: function(type) {
      ...
      if (this._events.hasOwnProperty(type)) {
        ...
        } else if (!(event.once && this._fired(event))) {
    ...
    _fire: function(event, args) {
      event.memory = event.memory ? args : false;
      ...
      if (event.callbacks.length) {
        ...
        for (; event.firingIndex < event.firingLength; event.firingIndex++) {
      ...
      if (!event.once) {
        if (event.stack.length) {
          event.memory = event.stack.shift();
          this.emit(event.memory);
        }
      } else {
        event.callbacks = [];
      }
    ...
    _fired: function(event) {
      if (typeof(event) === 'string')
        event = this._events[event];
      
      return (typeof(event.memory) === 'object');
    }
