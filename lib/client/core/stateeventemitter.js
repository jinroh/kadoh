...
(function(exports) {
  ...
  var Deferred = KadOH.core.StateEventEmitter = EventEmitter.extend({
    ...
    initialize: function() {
      this.supr();
      this.state = null;
      this._events.statechange = new KadOH.Event(false, false); // !memory && !once
    },
    ...
    /**
     * Attach a callback to the event corresponding of changing statte. Callback willl be called with state as argument.
     * @param  {Function} listener Callback to call
     * @param  {Object}   [scope]  Context of callback
     * @return {self} this
     */
    onStateChange: function(listener, scope) {
      this.on('statechange', listener, scope);
      return this;
    },
