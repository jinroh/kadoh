  ...
  KadOH.RoutingTable = EventEmitter.extend(
    ...
    addPeer: function(peer) {
      ...
      try {
        ...
        KadOH.log.info('RoutingTable','add peer', peer.getAddress(), peer.getID());
        ...
        if (e === 'full') {
          ...
          if (kbucket.isSplittable()) {
            ...
            this.emit('splitted');
            KadOH.log.info('RoutingTable','split kbucket', range, kbucket.getRange());
            this.addPeer(peer);
          } else {
            KadOH.log.info('RoutingTable','update kbucket', kbucket.getRange(), peer.getAddress(), peer.getID());
            this.emit('update', kbucket, peer);
          }
        } else if (e === 'updating') {
          // if the kbucket is updating
          // do nothing
          KadOH.log.info('RoutingTable', 'adding a peer while updating the bucket');
        } else {
          throw e;
        }
      }
    ...
    imports: function(routing_table) {
      ...
      try {
        ...
        KadOH.log.fatal('RoutingTable', 'failed to import', routing_table);
        return false;
      }
    }
