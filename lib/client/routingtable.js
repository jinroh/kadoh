  ...
  KadOH.RoutingTable = EventEmitter.extend(
    ...
    addPeer: function(peer) {
      ...
      try {
        ...
        KadOH.log.info('add peer', peer.getAddress(), peer.getID());
        ...
        if (e === 'full') {
          ...
          if (kbucket.isSplittable()) {
            ...
            this.emit('splitted');
            KadOH.log.info('split kbucket', range, kbucket.getRange());
            this.addPeer(peer);
          } else {
            KadOH.log.info('update kbucket', kbucket.getRange(), peer.getAddress(), peer.getID());
