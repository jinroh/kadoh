  ...
  KadOH.RoutingTable = EventEmitter.extend(
    ...
    initialize: function(node) {
      if (node instanceof KadOH.Node) {
        this._node = node;
        this._parentID = node.getId();
      } else {
        this._parentID = node;
      }
      this._kbuckets = [new KadOH.KBucket(this._parentID)];
    },
    ...
    distance: function(id) {
      return KadOH.util.Crypto.distance(this._parentID, id);
    },
     ...
     * 
     * @param {String} id
     * @param {Number} [number = {@link globals._alpha}] The number of peers you want
    ...
    _kbucketIndexFor: function(id) {
      ...
      for (var kbIndex=0; kbIndex < this._kbuckets.length; kbIndex++) {
        if (this._kbuckets[kbIndex].distanceInRange(dist)) {
          return kbIndex;
        }
      }
      return -1;
