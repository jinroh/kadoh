...
(function(exports, Strophe) {
  ...
  if (!Strophe) {
    if ('object' == typeof console) 
      KadOH.log.warn('WARNING : Strophe not defined : no transport');
    return;
  }
  ...
  KadOH.transport.XMPP = EventEmitter.extend({
    ...
    connect: function() {
      var self = this;
      this.setStateAndEmit('connecting', this._host, this._jid, this._password);
        ...
        function(status, error) {
          switch (status) {
            ...
            case Strophe.Status.ATTACHED:
              self.setStateAndEmit('connected', self._connection.jid);
              break;
            ...
            case Strophe.Status.CONNFAIL:
              self.setStateAndEmit('failed', error);
              break;
