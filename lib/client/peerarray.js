  ...
  var PeerArray = KadOH.PeerArray = Class({
    ...
    initialize: function(peers) {
      ...
      if (peers instanceof this.constructor)
        this._PEERS.push.apply(this._PEERS, peers.getRawArray());
      else if (peers)
        this.add(peers);
    },
...
// TRANSFORMing methods : these methods applly on the instance
    ...
    removePeer: function (rm_peer) {
      ...
      var i = this._PEERS.indexOf(rm_peer);
      ...
      if (i !== -1) {
        ...
        for (i = 0, l = this.length(); i < l; i++) {
          if (rm_peer.equals(this._PEERS[i])) {
            this._PEERS.splice(i, 1);
          }
        }
      }
...
//NON-TRANSFORMing methods : these methodes return a new instance of PeerArray
    ...
    pickOutFirst: function(number) {
      return new this.constructor(this._PEERS.slice(0, number));
    },
...
// Extends Array.prototype
    ...
    map: function() {
      return new this.constructor(Array.prototype.map.apply(this._PEERS, arguments));
    },
    ...
    filter: function() {
      return new this.constructor(Array.prototype.filter.apply(this._PEERS, arguments));
    },
  ...
  var XORSortedPeerArray = KadOH.XORSortedPeerArray = SortedPeerArray.extend({
    ...
    pickOutFirst: function(number) {
      return new this.constructor(this._PEERS.slice(0, number), this._relativeNodeId);
    },
