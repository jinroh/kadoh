...
(function(exports) {
  ...
  var parseRPCMessage = function(raw) {
    if ('string' === typeof raw) {
      ...
      _removeWhiteSpace(raw);
      if (raw.getElementsByTagName('parsererror').length > 0)
    ...
    var documentElement = raw.documentElement;
    var id              = documentElement.getAttribute('id');
    var type            = documentElement.tagName;
    var obj             = {};
    ...
    if (type === 'methodCall') {
      var method = documentElement.getElementsByTagName('methodName')[0];
      ...
      //validate params
      var params = documentElement.getElementsByTagName('params')[0];
      ...
      var result = documentElement.firstChild;
  ...
  var buildErrorResponse = function(error, id) {
    ...
    var xml = document.implementation.createDocument('', 'methodResponse', null);
    xml.documentElement.setAttribute('id', obj.id);
  ...
  RPCMessage.prototype =  {
    ...
    setRPCID  : function(id) {
      id = id ? String(id) : '';
      this.id = id;
      this._xml.documentElement.setAttribute('id', id);
      return this;
    },
    /**
