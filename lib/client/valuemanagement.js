...
(function(exports, Lawnchair) {
  ...
  if (!Lawnchair) {
    KadOH.log.warn('Lawnchair not defined : non-persisant basic storage');
  ...
  KadOH.ValueManagement = EventEmitter.extend({
    ...
    initialize: function(node, options) {
      ...
      KadOH.log.info('ValueManagement', 'initialize'+ ((options && options.recover) ? ' with recover' : ''));
      if (config.recover) {
        this._recover();
      } else {
        this._store.nuke();
      ...
      KadOH.log.info('ValueManagement', 'initialized');
    ...
    save: function(key, value, exp) {
      ...
      if( KeyValue.exp > 0 && KeyValue.exp <= now) {
        KeyValue.now = now;
        KadOH.log.warn('ValueManagement','save failed : key-value already expired', KeyValue);
      ...
      KadOH.log.info('ValueManagement', 'save', KeyValue);
     ...
     * @param  {Object}   [scope = this ValueManagement obejct] the scope to apply to the callback function (by default : the instance of ValueManagement)
      ...
      //check before if the key/value still exists
        this._store.exists(kv.key, function(exists) {
          if(exists) {
            ...
            KadOH.log.info('ValueManagement', 'republish', kv);
          }
        });
      return this;
    },
      ...
      //check before if the key/value still exists
        this._store.exists(kv.key, function(exists) {
          if(exists) {
            ...
            KadOH.log.info('ValueManagement', 'expiration', kv);
            self.emit('expire', kv);
          }
        });
      return this;
