  ...
  KadOH.rpc.Reactor = StateEventEmitter.extend({
    ...
    sendRPCQuery: function(rpc) {
      if (this.stateIsNot('connected')) {
        ...
        KadOH.log.debug('send query', rpc, rpc.normalizeQuery());
      }
      return this;
    },
    ...
    sendRPCResponse: function(rpc) {
      if(this.stateIsNot('connected')) {
        ...
        KadOH.log.debug('send response', rpc, rpc.normalizeResponse());
      }
      return this;
    },
    ...
    handleNormalizedQuery: function(query, from) {
      ...
      if(!this.RPCObject[method]) {
        KadOH.log.warn('receive query method ' + query.method + ' not available');
        return;
      }
    ...
    handleNormalizedResponse: function(response, from) {
      ...
      if(!rpc) {
        ...
        KadOH.log.debug('received response', from, response);
