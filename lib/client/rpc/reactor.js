  ...
  KadOH.rpc.Reactor = StateEventEmitter.extend({
    ...
    sendRPCQuery: function(rpc) {
      if (this.stateIsNot('connected')) {
        ...
        KadOH.log.debug('Reactor', 'send query', rpc, rpc.normalizeQuery());
      }
      return this;
    },
    ...
    sendRPCResponse: function(rpc) {
      if(this.stateIsNot('connected')) {
        ...
        KadOH.log.debug('Reactor', 'send response', rpc, rpc.normalizeResponse());
      }
      return this;
    },
    ...
    handleNormalizedQuery: function(query, from) {
      ...
      KadOH.log.debug('Reactor', 'received query', from, query);
    ...
    handleNormalizedResponse: function(response, from) {
      ...
      if(!rpc) {
        KadOH.log.info('Reactor', 'response matches no request', from, response);
      } else {
        KadOH.log.debug('Reactor', 'received response', from, response);
