...
(function(exports) {
  ...
  KadOH.rpc.Reactor = StateEventEmitter.extend({
    ...
    handleNormalizedQuery: function(query, from) {
      var name = (this.RPCObject.hasOwnProperty(query.method)) ? query.method : '__default';

      if(typeof this.RPCObject(name) == 'undefined') {
        this.emit('receive query method '+query.method+' not available');
        return;
      } else {

        var rpc = this.RPCObject[name].parseQuery(query, from);
    ...
    handleNormalizedResponse: function(response, from) {
      ...
      if(typeof rpc == 'undefined')
        ...
        this.emit('receive RPC Response', rpc);
      }
      return this;
    },
