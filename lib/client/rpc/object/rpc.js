...
(function(exports) {
  ...
  KadOH.rpc.object.RPC = Deferred.extend({
    ...
    handleNormalizedQuery: function(query, quering_peer, specific_handler) {
      if(typeof quering_peer == 'undefined' || quering_peer !== null)
        this.setQuering(quering_peer);

      this.setId(query.id);
      this.method = query.method;

      if (typeof specific_handler === 'function')
        specific_handler.call(this, query, quering_peer);
      else {
    ...
    handleNormalizedResponse: function(response, specific_handler) {
      if(this.stateIsNot('progress')) {
        this.reactor.emit('rcv response already finished', this.getQueried().getAddress(),this, response);
        return;
      }
