...
(function(exports) {
  ...
  KadOH.KBucket = PeerArray.extend(
    ...
    touch: function(time) {
      ...
      time = ((this._refreshTime && this._refreshTime < now) || (time && time < now)) ? now : null;
      this._refreshTime = (time || now + globals.TIMEOUT_REFRESH) *
                          (1 + Math.floor(globals.TIMEOUT_REFRESH_WINDOW * Math.random()));
