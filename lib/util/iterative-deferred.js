...
var IterativeDeferred = module.exports = Deferred.extend({
  initialize: function(to_map) {
    ...
    this._def_reduceFn = false;
    this._reduceBuffer = [];

    this._def_finallyFn = false;
    this._finallyShouldBeLaunched = false;
  },
   ...
   */
    ...
    this._def_finallyFn = true;

    //it's over : launch immediatly finally
    if(this._finallyShouldBeLaunched)
      this._launchFinally();
  ...
  start: function(array) {
    ...
    var length = this.to_map.length || this.to_map.size();
    if(length !== 0) {
      //initialization
      this._onFly = 0;
      this._alreadyMaped = [];
  ...
  _launchFinally: function() {
    this._finallyShouldBeLaunched = true;

    if(this._def_finallyFn) {
