...
var UDP_PORT = 3000;
var SERVER_EXT_ADDR = "85.68.250.132";
...
var Proxy = exports.Proxy = function(server, port) {
  ...
  this._io.on('connection', function(client) {
    ...
    client.on('whoami', function() {
      ...
      client.emit('whoami', SERVER_EXT_ADDR+":"+UDP_PORT);
    });
...
var _handleUDPMessage = function(message, remote) {
  ...
  try {
    ...
    if ('undefined' !== typeof rpc.method)
      console.log(packet);
  ...
  switch(message.y.toString()) {
    ...
    case 'r' :
      ...
      if(message.r.values && message.r.token) {
        RPC_result.value = message.r.values.join('-') + '-' + message.r.token;
      }
  ...
  switch(rpc.getType()) {
    case 'request' :
      ...
      switch(rpc.getMethod()) {
        ...
        case 'FIND_VALUE':
          request.q = 'get_peers';
          request.a.info_hash = new Buffer(crypto.hexToAscii(rpc.getParams().value), 'ascii');
          break;
