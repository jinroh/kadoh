...
var bencodeToJSONrpc2 = exports.bencodeToJSONrpc2 = function(raw) {
  ...
  var message = bencode.decode(raw);
  var RPC; //to return at end
  var RPC_id = message.t.toString();
  ...
  switch(message.y.toString()) {
    ...
    case 'r' :
      ...
      RPC_result.id = message.r.id.toString('hex');
      ...
      if(message.r.nodes) {
        ...
        for(var off = 0;
          ...
          var addr = nodes.slice(off + NODE_SIZE, off + NODE_SIZE + ADDR_SIZE).toString('hex');
          var port = nodes.slice(off + NODE_SIZE + ADDR_SIZE, off + NODE_SIZE + ADDR_SIZE + PORT_SIZE).toString('hex');
      ...
      if(message.r.values && message.r.token) {
        ...
        for (var i = 0; i < values.length; i++) {
          var value = values[i];
          var addr = value.slice(0, ADDR_SIZE).toString('hex');
          var port = value.slice(ADDR_SIZE, ADDR_SIZE + PORT_SIZE).toString('hex');
        ...
        RPC_result.value = JSON.stringify(values);
      }
    ...
    //RPC type : REQUEST
      ...
      RPC_param.id = message.a.id.toString('hex');

      switch(message.q.toString()) {
        case 'find_node':
          RPC_method = 'FIND_NODE';
          RPC_param.target = message.a.target.toString('hex');
          break;
...
var JSONrpc2ToBencode = exports.JSONrpc2ToBencode = function(rpc) {
  ...
  switch(rpc.getType()) {
    case 'request' :
      request.y = 'q';
      request.a = {};
      request.a.id =  new Buffer(rpc.getParams(0).id, 'hex');

      switch(rpc.getMethod()) {
        case 'FIND_NODE':
          request.q = 'find_node';
          request.a.target = new Buffer(rpc.getParams(0).target, 'hex');
          break;

        case 'FIND_VALUE':
          request.q = 'get_peers';
          request.a.info_hash = new Buffer(rpc.getParams(0).target, 'hex');
          break;
    ...
    case 'response' :
      request.y = 'r';
      request.r = {};
      request.r.id = new Buffer(rpc.getResult().id, 'hex');
